...
def run(r):
    ...
    _expect_status(r, "malformed projwin (3 vals) -> 400",
                   f"/data?model=hrrr&product=wind_vector&format=gribjson&time={T}&projwin=1,2,3", 400)

    r.section("Path-injection / malformed input should return 400 (Sonar S2083)")
    _expect_status(r, "COG unknown product -> 400", f"/data?model=hrrr&format=cog&product=bogus&time={T}Z", 400)
    ...
    _expect_status(r, "COG malformed time -> 400", f"/data?model=hrrr&format=cog&product=wind_vector&time=2024-13-45T99:00:00Z", 400)
    ...
    _expect_status(r, "wind_vector gribjson -> 200", f"/data?model=hrrr&product=wind_vector&format=gribjson&time={T}", 200)
